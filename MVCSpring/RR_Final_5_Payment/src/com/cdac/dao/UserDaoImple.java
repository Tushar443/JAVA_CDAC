...
public class UserDaoImple implements IUserDao {
	...
	@Override
	public boolean updateUser(String seatType, int noOfPassengers, double totalPrice, int userId) {
		boolean b = hibernateTemplate.execute(new HibernateCallback<Boolean>() {

			@Override
			public Boolean doInHibernate(Session session) throws HibernateException {
				Transaction tr = session.beginTransaction();
				Query q = session.createQuery("update User set seat_type=?,no_of_pass=?,total_price=? where user_id=?");
